...
ipcMain.on('select-file', async (event, arg) => {
  ...
  if(canceled) {
    ...
    event.reply('select-file', filePaths[0]);
  }
});
...
ipcMain.on('download', async (event,arg) => {
  try {
    const success = await network.downloadModList(arg[0],arg[1]);
    console.log('Successfully started downloading mod list');
  } catch (e) {
    console.log(`could not download mod list`);
  }
});
